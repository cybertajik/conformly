...
PRIVILEGED_ROLES = frozenset({Role.OWNER, Role.ADMINISTRATOR, Role.COMPLIANCE_MANAGER})
...
def get_tenant_context(
    ...
    try:
        context = resolve_tenant_context(database, principal, tenant_id)
        if context.role in PRIVILEGED_ROLES and not principal.mfa_verified:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="MFA enforcement: privileged role requires multi-factor authentication",
            )
        return context
